...
``/methodology/``  ``/status/``  ``/data/``  ``/api/v1/``  ``/feed.xml``  ``/_headers``  ``/_redirects``
...
def _write_redirects(out: Path) -> Path:
    """Cloudflare Pages ``_redirects``: send the bare ``/api/`` to the current API version."""
    body = """# Generated by castcheck.site.build — do not edit by hand.
/api /api/v1/ 301
/api/ /api/v1/ 301
"""
